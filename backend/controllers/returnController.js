...
const createReturn = async (req, res) => {
    ...
    // =================================================================
    // CORRECCIÓN: Declaramos epsilon aquí para que todos la vean
    // =================================================================
    const epsilon = 0.001; 
    ...
    try {
        ...
        // 4. Determinar Tipo de Reembolso
        if (total_refunded !== undefined) {
            // CASO A: Reembolso Manual
            final_total_refunded = parseFloat(total_refunded);

            if (final_total_refunded > original_order_total) {
                throw new Error(`El reembolso solicitado excede el total original.`);
            }

            const [prevRefunds] = await connection.execute(
                `SELECT SUM(total_refunded) as totalPreviouslyRefunded FROM returns WHERE order_id = ? AND status != 'cancelled'`,
                [order_id]
            );
            ...
            if ((totalPreviouslyRefunded + final_total_refunded) > (original_order_total + epsilon)) {
                const maxAllowed = original_order_total - totalPreviouslyRefunded;
                throw new Error(`El monto máximo a reembolsar es ${maxAllowed.toFixed(2)}.`);
            }
            ...
            for (const item of items) {
                ...
                if (orderItems.length === 0) {
                    throw new Error(`El item ${order_item_id} no pertenece al pedido ${order_id}.`);
                }
                ...
                if ((Number(totalReturnedForItem) + Number(quantity)) > ordered_quantity) {
                    throw new Error(`Error en producto '${originalItem.product_name}': Cantidad excesiva.`);
                }
                ...
                validatedItemsToInsert.push({
                    ...
                    quantity: quantity,
                    unit_price_refunded: parseFloat(originalItem.unit_price)
                });

                final_total_refunded += (parseFloat(originalItem.unit_price) * quantity);
            }

             // Validación final de montos (Usando la variable epsilon que ahora SÍ existe)
             const [prevManualRefunds] = await connection.execute(
                `SELECT SUM(total_refunded) as totalPreviouslyRefundedManual 
                 FROM returns 
                 WHERE order_id = ? AND status != 'cancelled' AND id NOT IN (SELECT DISTINCT return_id FROM return_items WHERE return_id IS NOT NULL)`,
                 [order_id]
             );
             ...
             if ((totalPreviouslyRefundedManual + final_total_refunded) > (original_order_total + epsilon)) {
                 throw new Error(`El reembolso total calculado excede el total original del pedido.`);
             }
        }
        ...
        if (items) {
            for (const itemToInsert of validatedItemsToInsert) {
                await connection.execute(
                    ...
                    [newReturnId, itemToInsert.order_item_id, itemToInsert.product_id, itemToInsert.product_name, itemToInsert.quantity, itemToInsert.unit_price_refunded]
                );
            }
        }
